...
- **Exit 2** (or script crash/traceback): Infrastructure error. Log the warning and proceed to Step 5.45. This check is advisory and must not block PRD generation.
...
If `existing_decisions` is empty or `new_decisions` is empty, proceed silently to Step 5.45.
...
If no issues found, proceed silently to Step 5.45.
...
For batch_num, batch in enumerate(batches):
    ...
    For decision in batch:
        questions.append({
            "header": decision.id,  # e.g., "ADR-F001" (max 12 chars)
